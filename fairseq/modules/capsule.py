...
class ContextualCapsuleLayer(CapsuleLayer):
    ...
    #    new_masked = inputs_u.new_zeros(length,num_in_caps).masked_fill(mask = new_times, value = float(-np.inf))
        ...
        for i in range(self.num_iterations):
            ...
            probs = logits_b[:,None,:,:,:].transpose(-1,1).squeeze(-1).masked_fill(mask=new_times,value=float(-np.inf))
...
def convert_to_past_labels(labels,New_type=None, padding_idx=1):
    ...
    # [batch, seq_len, seq_len]
    step_mask = torch.tril(labels.new_ones(seq_len, seq_len), -1).byte()
...
def convert_to_future_labels(labels, new_times, src_tokens, New_type=None,padding_idx=1,cross_mask=None):
    """
    Args:
        padding_idx:
        labels: [batch, seq_len]
    ...
    # use upper triangle to masking in descending manner
    # [batch, seq_len, seq_len]
    step_mask = 1 - cross_mask.float()  #torch.tril(labels.new_ones(seq_len, src_len), new_times-1).byte()
    mask = step_mask.unsqueeze(0) * seq_mask.unsqueeze(1)
    tmp_mask = labels.eq(padding_idx)
    mask = mask.masked_fill(tmp_mask.unsqueeze(-1), 0)
    scores = get_average_score(mask,New_type)
