...
class CapsuleLayer(nn.Module):
    ...
    def forward(self, x, decoding_hid, new_times, encoder_mask=None):
        x = torch.transpose(x, 0, 1)  #[batch * srclen * dim]
        src_len = x.size(1)
        tgt_len = decoding_hid.size(1)
        batch_size = x.size(0)
        capsule_num = self.num_capsules

        priors = self.capsule_project(input=x)  # batch * srclen * capsule * dim
        ...
        logits = x.new_zeros((batch_size, tgt_len, src_len, capsule_num))  # batch * tgtlen * srclen * capsule
        if encoder_mask is not None:
            encoder_mask = encoder_mask[:, None, :, None].expand_as(logits)
        
        '''     
        new_priors = priors[:,None,:,:,:].repeat(1,tgt_len,1,1,1)  # batch * tgtlen * srclen * capsule * dim
        new_decoding_hid = decoding_hid[:, :, None, None, :].repeat(1, 1, src_len, capsule_num, 1) # batch * tgtlen * srclen * capsule * dim
        '''
        ...
        for i in range(self.num_iterations):
            ...
            outputs = self.squash((probs.unsqueeze(-1) * priors.unsqueeze(1)).sum(2)) # batch * tgtlen * capsule * dim，6968->7360，9586->9606,11832->11852

            if i != self.num_iterations - 1:
                delta_logits = self.compute_delta_sequence(priors, outputs, decoding_hid)
                logits = logits + delta_logits
                '''
                new_outputs = outputs[:,:,None,:,:].repeat(1, 1, src_len, 1, 1)  # batch * tgtlen * srclen * capsule * dim
                tmp_res = torch.cat((new_decoding_hid, new_priors, new_outputs), -1) # batch * tgtlen * srclen * capsule * dim
                tmp_res = torch.tanh(torch.matmul(tmp_res, self.weights_b))
                delta_logits = torch.matmul(tmp_res, self.w.reshape(-1, 1)).squeeze(-1) # batch * tgtlen * srclen * capsule * 1
                logits = logits + delta_logits
                '''
        return outputs  # batch * tgtlen * capsule * dim